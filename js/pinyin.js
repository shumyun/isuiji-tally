...
var pinyin = function(){
    ...
    for(var i=0,l=DICT.length; i<l; i++){
        ...
        for(var j=0,m=hans.length; j<m; j++){
            ...
            PINYIN_DICT_CACHE[han].push({data: DICT[i][0], num: j});	//存储汉字的数据
        }
    }
    
    for(var han in PINYIN_DICT_CACHE){
    	PINYIN_DICT_CACHE[han].sort(function(a, b){
    		return a.num - b.num;  
    	});
    }
    ...
    return function(hans, single, sp){
        ...
        if(len==1){
            ...
            if(single){return y&&y[0]?y[0].data:hans;}
            return y || [hans];
        }else{
            var py = [];
            for(var i=0,y; i<len; i++){
                ...
                if(y){
                	if(single){
                        py[py.length] = y[0].data;
                	} else {
                		var length = py.length;
                		py[length] = new Array();
                		for(var z in y){
                			py[length].push(y[z].data);
                		}
                	}
                }else{
